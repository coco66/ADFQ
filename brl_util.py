...
def posterior_adfq_batch_helper(target_means, target_vars, c_mean, c_var, discount, scale_factor, asymptotic=False):
    ...
    for k in range(target_means.shape[0]):
        stats = posterior_adfq_helper(target_means[k], target_vars[k], c_mean[k], c_var[k], discount, scale_factor, asymptotic=False)
...
def plot_IQR(T, data, labels, x_label='x', y_label='y', x_vals = None, title=None, save=False, legend=(True, (1,1)), shadow=True, pic_name = None, colors=None,):
    ...
    if len(data.shape)==2:
        ...
        for (i,y) in enumerate(data):
            power_smooth = spline(x_val,[0]+list(y),xnew)
            power_smooth = [max(0,el) for el in power_smooth]
            tmp, = ax.plot(xnew, power_smooth, colors[i], label=labels[i], linewidth=2.0)
            plot_err.append(tmp)
    else:
        if x_val == None:  
            x_val = range(0,T) #range(0,T-1,T/N)
        for (i,y) in enumerate(data):
            m, ids25, ids75  = iqr(y)
            tmp, = ax.plot(x_val, m, MARKER[i+2], color=colors[i], markeredgecolor = colors[i], markerfacecolor='None', label=labels[i], linewidth=2.0)
            plot_err.append(tmp) 
            if shadow:       
                ax.fill_between(x_val, list(ids75), list(ids25), facecolor=colors[i], alpha=0.15)

    if legend[0]:
        ax.legend(plot_err, labels ,loc='upper right', shadow=True,
                    prop={'family':'Times New Roman', 'size':16})#, bbox_to_anchor=legend[1])
    ax.tick_params(axis='both',which='major',labelsize=15)
    ax.set_xlabel(x_label,fontsize=16, fontname="Times New Roman")
    ax.set_ylabel(y_label,fontsize=16, fontname="Times New Roman")
    ax.grid()
    if title:
...
def plot_smoothing(T, data, labels, x_label='x', y_label='y', title=None, save=False, legend=(True,(0.8,0.5)), pic_name=None, window=4, colors=None):
    import matplotlib.pyplot as plt
